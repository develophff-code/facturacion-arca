...
class AfipFacturacion:
    ...
    def __init__(self, config: dict, base_dir: str = None):
        ...
        for env in ("homo", "prod"):
            if env in config.get("certificados", {}):
                self.certs[env] = {
                    "cert": os.path.join(self.base_dir, config["certificados"][env]["cert"]),
                    "key":  os.path.join(self.base_dir, config["certificados"][env]["key"]),
                }
    ...
    def emitir_factura(self, factura_json: dict) -> dict | None:
        ...
        last_num = self._get_last_voucher_number(wsfe, token, sign, pto_venta, tipo_cbte)
        next_num = last_num + 1
    ...
    def _create_login_ticket_request(self, service: str = "wsfe") -> str:
        ...
        exp_time  = (now + datetime.timedelta(minutes=10)).strftime("%Y-%m-%dT%H:%M:%S-00:00")
...
<loginTicketRequest version="1.0">
  ...
  <service>{service}</service>
    ...
    def _get_last_voucher_number(self, client, token: str, sign: str,
                                  pto_venta: int, tipo_cbte: int) -> int:
        auth = {
            'Token': token,
            'Sign':  sign,
            'Cuit':  int(self.cuit_emisor),
